...
import { filePathExistsOrThrow, isAstroFile, isCssFile, loadJsonFile } from "../../cli/_shared/fs.js"
import { getProjectConfig, loadNativeElementsJson } from "../../cli/_shared/validation.js"
...
export function makeLspTools({
	...
	try {
		...
		filePathExistsOrThrow({
			...
			kind: "generated log file",
			description: "Please run the `compile` command, then reload the language server."
		})
		...
		function _convertToLSPDiagnostics(diagnostics: Diagnostic[]): LSPDiagnostic[] {
			return diagnostics.map((d) => {
				const lspDiag: LSPDiagnostic = {
					...
					source: GLOBALS.HAQ_ASTRO_OFFICIAL_NAME
				}
				return lspDiag
			})
		}
		...
		async function getCompletions({
			...
			filePath,
			cursorPos
		}: ARGS_getCompletions): Promise<CompletionItem[]> {
			if (!(isAstroFile(filePath) || isCssFile(filePath))) return []
		...
		if (e instanceof HAQError) {
			...
			errorCallback(fullMessage)
			console.error(fullMessage)
		}
		return Object.freeze({
			getFileDiagnostics: async (_args: ARGS_getFileDiagnostics) => [],
